<template>
  <footer>
    <div class="bottomControl w">
      <div class="left">
        <img class="avatar" v-lazy="left.imgUrl + '?param=55y55'" alt="无法显示" />
        <div class="info">
  ...
  methods: {
    ...
    changePlaylisy(newPlaylist, playlistId) {
      ...
      if (this.playlistId !== playlistId) {
        ...
        this.playlist = newPlaylist // 保存新的播放列表
      }
    },
    ...
    addSong(song) {
      ...
      let exist = this.playlist.find((item) => {
      ...
      if (exist) {
        ...
        let index = this.music.playSongIndex || 0 // 可能播放列表没有歌曲 就为0
